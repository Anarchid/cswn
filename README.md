...
One sprite comes from outside that lineage: `sprites/nuke_scar.png`, the
...
So do the nine `sounds/*.ogg` — the dry-fire clicks, reload and
empty-magazine sounds, and the plasma discharge. They are cut from
freesound.org recordings under CC0 or CC BY 4.0, and every one is credited by
title, author and licence in [`sounds/origin.txt`](sounds/origin.txt), which
is also where the reasoning lives for the one sound from that batch this
package does **not** ship: a CC BY-NC recording, whose NonCommercial term the
GPL cannot honour.
